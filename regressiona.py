...
for train_index, test_index in CV.split(X):
    ...
    if len(selected_features) == 0:
        ...
        subplot(1,3,3)
        bmplot(attrnames, range(1,features_record.shape[1]), -features_record[:,1:])
        clim(-1.5,0)
        xlabel('Iteration')
...
figure(k)
subplot(1,3,2)
bmplot(attrnames, range(1,Features.shape[1]+1), -Features)
clim(-1.5,0)
...
if len(ff) == 0:
    ...
    for i in range(0,len(ff)):
       subplot(2, int(len(ff) // 2 + 1),i+1)
       plot(X[:,ff[i]],residual,'.')
       xlabel(attrnames[ff[i]])
